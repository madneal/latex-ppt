...
\renewcommand{\figurename}{图} %重命名图例下标
    ...
    \end{itemize}
    \item 交叉操作：利用交叉算子结合变异向量和目标向量来产生实验向量。
    ...
    \begin{frame}
    差分进化自从提出后，就受到了工业界以及学业界的广泛专注。根据差分进化解决的问题类型，可以将差分进化分为以下几种类型：
    \begin{itemize}
    \item 针对连续单目标优化问题的差分进化算法
    \item 针对约束优化的差分进化算法
    \item 针对多目标优化问题的差分进化算法
    \end{itemize}
    \end{frame}
    ...
    \frame{\tableofcontents[currentsection,currentsubsection,subsectionsonly]}
    \begin{frame}
    单目标优化问题几乎出现在科学和工程应用的各个领域中。单目标优化问题的目标函数一般是非凸函数，并且在可行区域内具有很多的局部极小值或者极大值。本文研究的单目标优化问题针对的是连续空间的全局优化问题，即是求目标函数的最小值或者最大值。
    ...
    \begin{column}{0.5\textwidth}
    \begin{itemize}
    \item DE-EIG算法是基于特征向量的差分进化算法，其主要贡献是在一个旋转的坐标空间对个体进行交叉操作，这样可以利用旋转空间中种群的协方差矩阵的特征向量信息。
    ...
    \begin{column}{0.5\textwidth}
    \begin{itemize}
    \item DE/EDA是一种结合差分进化和分布估计算法来解决全局连续优化问题。
    \item DE/EDA通过利用分布估计算法可以提取种群全局信息和差分进化可以提取种群查分信息的优点，是一种非常具有研究前景的算法。
    \end{itemize}
    \end{column}
    \end{columns}
    \end{frame}
    ...
    \begin{columns}
    \begin{column}{0.5\textwidth}
    利用$Converage(\theta,t,t_e)$函数来判断解是否收敛
    ...
    \item 通过将$min\{\Delta{f}, \Delta{x}\}$和给定的阈值$\theta$进行比较，皆可以判断解是否收敛。为了提高算法的运行效率，每两次expensive LS至少间隔50代。
    \end{itemize}
    \end{column}
    \end{columns}
    \end{frame}
    ...
    \begin{column}{0.5\textwidth}
    \begin{itemize}
    \item 为了增加种群的多样性，通过利用随机参数来设置$CRP$，可以提高算法对于大多数问题的鲁棒性。
    \item 在基于DE/EDA算法框架的基础上，利用DE-EIG进行采样，同时结合expensive LS 进一步提高解集质量，提出基于差分采样的单目标分布估计算法。
    \end{itemize}
    \end{column}
    \end{columns}
    \end{frame}
    ...
    \begin{frame}
    \begin{itemize}
    \item 三个算法在$f6$测试题上都收敛地很早，因此不用折线图展示。
    \item 和DE/EDA相比，EDA/DE-EIG具有较大的领先，在10个测试题上都具有较大幅度优势（除了$f7,f8$）
    \item 和JADE相比，在$f1,f2,f3,f4,f11$ 5个测试题上在下降趋势和最终结果都有明显优势，在$f10,f12,f13$三个测试题上，下降趋势更佳，对于$f5$虽然最终结果不如JADE，但是下降趋势表现得更好，在$f7,f8,f9$ 3个测试题上表现得不如JADE。
    \end{itemize}
    \end{frame}
    \begin{frame}
    \begin{figure}
    ...
    \begin{frame}
    在多目标优化问题中，多个目标相互之间往往是冲突的，从而导致无法在满足所有约束条件下使得所有目标函数都能够达到全局最优解，但是存在一组Pareto最优解。

    对于此，做出以下定义：令$a,b\in{R^n}$，当$a_i\le{b_i}\land{a\neq{b}}$，且$i=1,\cdots,n$，则称$a$ 支配$b$。向量$x^*\in\Omega$ 即是公式~\ref{mop}的Pareto 最优解，如果不存在$x\in\Omega$ 使得$F(X)$ 支配$F(x^*)$。$F(x^*)$被称为Pareto最优目标向量。所有的Pareto 最优解的集合就是Pareto最优解集（PS），对应的最优向量的集合则成为Pareto前端（PF）。
    \end{frame}

    \begin{frame}
    在一般条件下，根据Karush-Kuhn-Tucker 可以推导出：连续多目标问题在决策空间中的Pareto set 是一个连续分段的（m-1）维的流形体（m是目标数）。对于一个成功的多目标演化算法（multiobjective evolutionary algorithm, MOEA）来说，独立的个体应该是在决策空间中分散在Pareto set 附近，如图~\ref{FIG1} 所示。
    \begin{figure}[ht]
    \graphicspath{{figs/}}
    \centering
    ...
    \begin{column}{0.5\textwidth}
    \begin{itemize}
    \item 在每一个聚类之中，$N^k$用来表示聚类中的Pareto set，$M^k$则用来覆盖每个聚类中的Pareto set。
    \item 为了能够覆盖聚类中的Pareto set，RM-MEDA通过设置一个缩放比例用来覆盖聚类中的Pareto set。但是这个缩放比例依赖于具体的问题，对于不同的问题，其表现也不尽相同。
    \end{itemize}
    \end{column}
    \end{columns}
    \end{frame}
    ...
    \begin{column}{0.4\textwidth}
    \begin{itemize}
    \item 如果不设置缩放因子，那么RM-MEDA表现则不是很好
    \item 缩放因子设置的越大，则RM-MEDA表现得也更加优秀
    \item 对于设置较大的缩放因子也可能会造成性能的不稳定
    \item 总的来说，如何在实践中设置一个最佳的缩放因子还是比较困难的。
    \end{itemize}
    \end{column}
    \end{columns}
    \end{frame}
    ...
    \begin{frame}
    \begin{itemize}
    \item 本文针对分布估计算法中的采样，提出了差分采样策略，对于单目标优化问题和多目标优化问题分别提出了基于差分采样的单目标分布估计算法和基于差分采样的多目标分布估计算法。通过综合的实验分析对比，差分采样策略对于提高分布估计算法性能具有重大意义。
    \textcolor{red}{\hrule}
    \end{itemize}
    ...
    \item 对于DE/EDA算法中，分布估计算法和差分进化的资源分配是一个值得继续探索的话题
    \end{itemize}
    \end{frame}
